...
class History
  ...
  # Example:
    # input == array of move objects:
      # moves ==    [#<Move:0x0000000305dce8 @value="scissors">,
                     #<Move:0x00000002f04450 @value="paper">]
      # outcomes == [:win, :lose]
    # function:
...
class Move
  ...
  def >(other)
    WINNING_CONDITIONS[value].include?(other.value)
  end

  def battle_msg(other)
    moves = [value, other.value].sort
...
class Hal < Computer
  MOVE_WEIGHTS =
    ['rock'] * 15 + ['scissors'] * 65 + ['lizard'] * 5 + ['zpock'] * 15
