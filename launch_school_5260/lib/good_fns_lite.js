...
//********************** !!!!!!!!! **********************\\
Object.defineProperty(Array.prototype, 'lineNum', {
  get() {
    if (!this[1] || !this[1].err) return
    console.log(this[1].err)
    return this[1].err
  },
});
const getLineNum = (idx = 3) => {
  let err     = new Error('lineNum')
  let lineNum = (err.stack.split('\n'))[idx].match(/[^/]+$/)[0].replace(/\)$/, '')
  if (lineNum.includes('anonymous')) return ''

  let lineNumWrapper = [`___${lineNum}___`, { err }]
  return lineNumWrapper

  // return `___${lineNum}___`

  // let msg     = `%c${lineNum}`;
  // let style   = 'color: #33b5e5; font-size: 8px; background: #000;';
  // console.log(msg, style);
}

//========================= p -- print =======================================//
const p = (...args) => {
  let lineNum = getLineNum(3)
  args.length > 0 ? console.log(...args, lineNum) : console.log('', lineNum);
...
Object.defineProperty(Object.prototype, 'p', {
  get() {
    let lineNum = getLineNum(3)
    console.log(this.valueOf(), lineNum)
    return this.valueOf()
  },
});
//========================= p -- print =======================================//
