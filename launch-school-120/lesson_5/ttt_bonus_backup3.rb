...
class Computer < Player
  ...
  def mark(board)
    square_num = minimax(board, MARKER)[:square_num]
    board[square_num] = MARKER
  end

  def move_score(new_board, depth)
    case new_board.winning_marker
  ...
  def minimax(new_board, marker, depth = 0)
    return move_score(new_board, depth) if new_board.end_state?
    ...
    new_board.empty_square_nums.each do |square_num|
      ...
      result = minimax(new_board, opponent_marker, depth + 1)
    ...
    raise 'ERROR' unless new_board.empty_square_nums.size == moves.size
