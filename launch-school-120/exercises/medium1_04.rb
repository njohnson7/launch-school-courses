...
class CircularQueue
  ...
  def enqueue(item)
    @age += 1
    queue[index_to_update] = { age: @age, item: item}
  end

  def dequeue
    oldest = oldest_item
    oldest && queue[oldest_item_index] = nil
    oldest && oldest[:item]
  end
  ...
  def oldest_item
    empty? ? nil : queue.compact.min_by { |hsh| hsh[:age] }
  end

  def oldest_item_index
    queue.index(oldest_item)
  end
