...
# as well as a minor optimization technique, see the Bubble Sort wikipaedia
# page.
...
def bubble_sort!(ary)
  loop do
    ...
    ary.each_with_index do |e, idx|
      break if idx == ary.size - 1
      ary[idx, 2] = ary[idx + 1], e if e > ary[idx + 1]
    end
    break if ary_copy == ary
  end
  ary
end
...
p bubble_sort!((1..50).to_a.shuffle)
puts
...
def bubble_sort!(ary)
  ...
  while size > 0
    ...
    1.upto(size - 1) do |idx|
      next if ary[idx - 1] <= ary[idx]
      ary[idx - 1, 2] = ary[idx], ary[idx - 1]
      last_swap_idx = idx
    end
    size = last_swap_idx
  end
