...
- "and defaulted it to [], we have to be "
- rephrase: " In our example, if we stored lettersGuessed array on the Game object and defaulted it to [], we have to be very careful that our created games don't mutate this array, by calling push on it, for example. "
- "Otherwise one"
- " a piece of data ... initialize them "
...
-----
- "re-implement"
- " if we stored lettersGuessed array on "
- semi: `var game = Object.create(Game).init()`
- refactor:
  - `without` function
  - `var spaces = (new Array(this.word.length + 1)).join('<span></span>');`
  - `var self = this;`
  - `aCode, zCode`
  - `this.word` is a confusing var name for an array of characters
  - `if (!this.word) ... return this;`
  - 2 calls...: `this.toggleReplayLink(false);`
  - wtf...: ` var letter = String.fromCharCode(e.which); ... if (notALetter(e.which))`
  - `processGuess`
  - etc...
