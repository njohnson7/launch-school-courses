...
(function () {
  let dict = [{ foo: 'bar', quux: 'q', idx: 0 }, { foo: 'baz', quux: 'z', idx: 1 }, { foo: 'bar', quux: 'z', idx: 2 }];
...
(function () {
  ...
  test('findWhere returns undefined with no matched properties', function () {
    return _(dict).findWhere({ foo: 'quux' }) == null;
  });
}());
...
test('pick returns a new object with the passed in properties', function () {
  let old_obj = { foo: 'bar' };
  let new_obj = _(old_obj).pick('foo');
...
test('pick ignores any properties passed in that do not exist on the source object', function () {
  let old_obj = { foo: 'bar' };
  let new_obj = _(old_obj).pick('foo');
  return _(new_obj).pick('foo', 'bar').bar == null;
});
test('pick only picks properties passed as arguments', function () {
  let old_obj = { foo: 'bar', b: 'B' };
  let new_obj = _(old_obj).pick('foo');
  return new_obj.b == null;
});
...
//=============================== _.randomNum ================================//
test('randomNum exists', function () {
  return typeof _.randomNum == 'function';
});
test('randomNum creates a random number between min and max', function () {
  let _randomNums = _([...new Set([...Array(1000)].map(() => _.randomNum(5, 51)))].sort((a, b) => a - b));
  return _randomNums.first() === 5 && _randomNums.last() === 50 && _randomNums.length == 46;
});
test('randomNum creates a random number betwen 0 and max when passed a single argument', function () {
  let _randomNums = _([...new Set([...Array(100)].map(() => _.randomNum(5)))].sort((a, b) => a - b));
  return _randomNums.first() === 0 && _randomNums.last() === 4 && _randomNums.length == 5;
});
test('randomNum returns 0 if no arguments are passed', function () {
  let _randomNums = _([...new Set([...Array(10)].map(() => _.randomNum()))]);
  return _randomNums.first() === 0 && _randomNums.length == 1;
});

//=============================== _.prototype.length ================================//
test('length getter returns length', function () {
  return _().length === 0 && _([1]).length === 1 && _([1, 1, 1]).length === 3;
});
